const fs = require('fs');
const content = fs.readFileSync('src/services/data-service.js', 'utf8');

const replacement = `async function getMockCommercialPerformance(propertyId, filters) {
  await delay(200)
...
const newContent = content.replace(/async function getMockCommercialPerformance\([\s\S]*?affiliatesWithHealth,\n  }\n}/, replacement);

if (newContent === content) {
  console.log("NO CHANGE MADE. REGEX DID NOT MATCH.");
} else {
  fs.writeFileSync('src/services/data-service.js', newContent);
